...
router.get('/:id', (req, res) => {
  ...
  Category.findOne({
    ...
    include: [Product],
  }).then(singleCategory => res.json(singleCategory)).catch(err => res.status(400).json(err));
});
...
router.put('/:id', (req, res) => {
  ...
  }).then(updatedCategory => res.json(updatedCategory)).catch(err => res.status(400).json(err));
});
